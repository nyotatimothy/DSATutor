    ...
    try {
      const response = await fetch('/api/ai/learning-path', {
        ...
        body: JSON.stringify({
          ...
          userScore: solution.score || 0,
          userPerformance: solution.detailedFeedback?.performance || {},
          userCode: code,
          language: selectedLanguage
        }),
      });
      ...
      if (response.ok) {
        const data = await response.json();
        const nextProblemId = data.nextProblemId || (parseInt(params.id as string) + 1);
        window.location.href = `/problems/${nextProblemId}/solve`;
      } else {
        // Fallback to next problem in sequence
        const nextProblemId = parseInt(params.id as string) + 1;
        window.location.href = `/problems/${nextProblemId}/solve`;
      }
    } catch (error) {
            ...
            <Card className="bg-slate-50 border-slate-200">
              ...
              <CardContent>
                <Textarea
                  ...
                  className="min-h-[400px] font-mono text-sm bg-slate-100 border-slate-300 focus:bg-slate-50"
